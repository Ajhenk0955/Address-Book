...
public class SimpleButtonFileDialog extends JButton {
	...
	public SimpleButtonFileDialog(){
	}
	/** Opens prompt window that allows the user to navigate
	 * through files to the desired file to save to
	 * 
	 * @return Path of the user Selected file 
	 */
	public Path toSaveFilePath( Component target ){
		JFileChooser fileFinder = new JFileChooser();
		fileFinder.setCurrentDirectory(new File(System.getProperty("user.home")));
		
		if(fileFinder.showSaveDialog(this) == JFileChooser.APPROVE_OPTION){
			return fileFinder.getSelectedFile().toPath();
		}else{
			return null;
		}
	}
	/** Opens prompt window that allows the user to navigate
	 * through files to the desired file to Load from
	 * 
	 * @return
	 */
	public Path toLoadFilePath(){
		JFileChooser fileFinder = new JFileChooser();
		fileFinder.setCurrentDirectory(new File(System.getProperty("user.home")));
	 ...
	 * @return
	 */
	public Path toLoadFilePath( String[] extension_types , String description){
		JFileChooser fileFinder = new JFileChooser();
	 ...
	 * @return
	 */
	public Path toSaveFilePath(String[] extension_types , String description){
		JFileChooser fileFinder = new JFileChooser();
		fileFinder.addChoosableFileFilter(new SimpleFileFilter(extension_types, description));
		fileFinder.setCurrentDirectory(new File(System.getProperty("user.home")));
